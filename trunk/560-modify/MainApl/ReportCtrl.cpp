...
void CReportCtrl::_MouseClkMonitor(UINT nMsg, UINT nFlags, CPoint point, BOOL bTriggerEdit)
{
	...
	if(_IsURL(IDX, hti.iSubItem) && _PtInText(point, IDX, hti.iSubItem) )
	{
		...
		//HWND hwnd = (CWLRClientDlg *)AfxGetApp()->GetMainWnd()->GetSafeHwnd();
		::SendMessage(m_servicehwnd,WM_REQUEST,0,(WPARAM)hti.iItem);
	}
}
...
void CReportCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	try
	{
		...
		//static LVHITTESTINFO PreHti;有bug，如果上次悬停的项没有恢复，则下次打开会报错
		//static BOOL PreIn;
		NowHti.pt = point;
