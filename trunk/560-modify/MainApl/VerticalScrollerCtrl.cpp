// CVerticalScrollerCtrl : class implementation
...
const int   CVerticalScrollerCtrl::SC_SCROLL_COMPLETE = 0;

// defaults
const int   CVerticalScrollerCtrl::nSCROLL_DELAY  = 80;    // time between each frame (milliseconds)
const int   CVerticalScrollerCtrl::nSCROLL_PAUSE  = 5000;  // time to pause before autoscrolling (milliseconds)
const int   CVerticalScrollerCtrl::nMARGIN        = 5;     // (pixels)
const int   CVerticalScrollerCtrl::nFONT_SIZE     = 12;    // (points)
const int   CVerticalScrollerCtrl::nFONT_WEIGHT   = FW_SEMIBOLD;
const char* CVerticalScrollerCtrl::szFONT_NAME    = "Comic Sans MS";

// initialization
CVerticalScrollerCtrl::CVerticalScrollerCtrl()
{
...
BOOL CVerticalScrollerCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT uStyle, UINT nID)
{
   ...
   uStyle &= ~WS_VSCROLL;
	...
	{
      m_eState = PAUSED;
      SetTimer(201, m_nScrollDelay, NULL);
...
void CVerticalScrollerCtrl::SetWrapping(BOOL bWrap)
{
   m_bWrap = bWrap;
   if ( NULL != m_hWnd )
      Invalidate(FALSE);
...
void CVerticalScrollerCtrl::SetBgColor(COLORREF clrBg)
{
...
void CVerticalScrollerCtrl::SetFgColor(COLORREF clrBg)
{
...
void CVerticalScrollerCtrl::SetFont(const CString& strName, int nSize, int nWeight)
{
   if ( NULL != m_font.GetSafeHandle() )
      m_font.DeleteObject();
...
void CVerticalScrollerCtrl::SetText(const CString& strText)
{
   m_strText = strText;
   if ( NULL != m_hWnd )
   {
...
CBitmap* CVerticalScrollerCtrl::SetLogo(CBitmap* pbmpLogo)
{
...
CBitmap* CVerticalScrollerCtrl::SetPattern(CBitmap* pbmpPattern, BOOL bTile)
{
...
void CVerticalScrollerCtrl::SetScrollDelay(int nScrollDelay)
{
...
void CVerticalScrollerCtrl::SetScrollPause(int nScrollPause)
{
   m_nScrollPause = nScrollPause;
}

BEGIN_MESSAGE_MAP(CVerticalScrollerCtrl, CWnd)
	ON_WM_ERASEBKGND()
	ON_WM_GETDLGCODE()
	ON_WM_KEYDOWN()
	ON_WM_MOUSEWHEEL()
...
// CVerticalScrollerCtrl message handlers

// entire window is updated in OnPaint() so do nothing here.
BOOL CVerticalScrollerCtrl::OnEraseBkgnd(CDC* pDC)
{
	return FALSE;
}
...
void CVerticalScrollerCtrl::OnPaint()
{
	CPaintDC dc(this);
	CDC dcBackBuffer;
...
void CVerticalScrollerCtrl::RecalcLayout(CDC* pDC)
{
	CRect rectClient;
...
void CVerticalScrollerCtrl::FillBackground(CDC* pDC)
{
   CRect rectClient;
   GetClientRect(&rectClient);
...
int CVerticalScrollerCtrl::DrawLogo(CDC* pDC, int nOffset, BOOL bDraw)
{
   if ( NULL == m_pbmpLogo )
      return 0;
...
int CVerticalScrollerCtrl::DrawBodyText(CDC* pDC, int nOffset, BOOL bDraw)
{
...
UINT CVerticalScrollerCtrl::OnGetDlgCode()
{
...
void CVerticalScrollerCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
   if ( GetStyle()&WS_TABSTOP )
      SetFocus();
}
...
void CVerticalScrollerCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
   switch ( nChar )
   {
   case VK_UP:
...
void CVerticalScrollerCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
   if ( !(GetStyle()&WS_TABSTOP) )
      return;
...
BOOL CVerticalScrollerCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
...
void CVerticalScrollerCtrl::OnTimer(UINT nIDEvent)
{
